...
async function roCrateToJSON(crate, multiPageConfig, layout = []) {
  ...
  for (let e of crate.entities()){
    if (e["@type"].includes("rdf:Property")){
      ...
      crateLite.infoLinks[resolvedId] = encodeURIComponent(e["@id"]);
    }
  }

  crateLite.tabular = buildTabularData(crateLite, layout, multiPageConfig);
  crateLite.typeLabels = crateLite.tabular.typeLabel  ;
 ...
 *
 */
function renderTemplate(data, template, config = {}, css = "") {
  ...
  return env.renderString(template, { data, config, css });
}
