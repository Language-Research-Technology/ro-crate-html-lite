...
function getFirstForwardValue(entityLite, propUri, field = "value") {
  const prop = entityLite?.props?.[propUri];
  if (!prop || !Array.isArray(prop.fwd) || prop.fwd.length === 0) {
    return "";
  }
  return prop.fwd[0]?.[field] || "";
}
...
function resolvePreviewImage(entityId, crateLite, previewCache, visiting = new Set()) {
  if (previewCache.has(entityId)) {
    return previewCache.get(entityId);
  }
  ...
  const entityLite = crateLite.ids[entityId];
  if (!entityLite) {
    previewCache.set(entityId, null);
    return null;
  }

  visiting.add(entityId);

  const imageTarget = getFirstForwardValue(entityLite, "http://schema.org/image", "target_id");
  if (imageTarget) {
    const preview = {
      path: imageTarget,
      alt: stripHtml(
        getFirstForwardValue(entityLite, "http://schema.org/caption") ||
        getFirstForwardValue(entityLite, "http://schema.org/name") ||
        entityLite.id,
      ),
    };
    visiting.delete(entityId);
    previewCache.set(entityId, preview);
    return preview;
  }

  const parts = entityLite.props?.["http://schema.org/hasPart"]?.fwd || [];
  for (const part of parts) {
    if (!part.target_id) {
      continue;
    }
    const preview = resolvePreviewImage(part.target_id, crateLite, previewCache, visiting);
    if (preview) {
      visiting.delete(entityId);
      previewCache.set(entityId, preview);
      return preview;
...
export async function roCrateToJSON(crate, multiPageConfig) {
  ...
  const pageDomain = multiPageConfig?.domain ? multiPageConfig.domain : null;
  const crateLite = {
  ...
  for (const entityLite of Object.values(crateLite.ids)) {
    const preview = resolvePreviewImage(entityLite.id, crateLite, previewCache);
    if (preview) {
      entityLite.previewImage = preview.path;
      entityLite.previewImageAlt = preview.alt;
    }
  }
