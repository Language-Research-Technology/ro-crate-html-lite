...
function buildTermLabelResolver(config = {}) {
  ...
  function resolveFromEntry(entry) {
  ...
  return function resolveTermLabel(uri, fallbackLabel = "") {
    if (!uri || typeof uri !== "string") {
      return fallbackLabel || "";
    }

    const exact = resolveFromEntry(mapping[uri]);
    if (exact) {
      return exact;
    }
    ...
    const localName = getUriLocalName(uri).toLowerCase();
    if (localName && localNameFallback.has(localName)) {
      const fallback = resolveFromEntry(localNameFallback.get(localName));
      if (fallback) {
        return fallback;
      }
    }
...
function initializeProp(crate, entityLite, prop, resolvedUri = null, resolveTermLabel = null) {
  ...
  if (!entityLite.props[uri]) {
    entityLite.props[uri] = {
      fwd: [],
      rev: [],
      label: resolveTermLabel ? resolveTermLabel(uri, prop) : prop,
    };
  }
...
async function roCrateToJSON(crate, multiPageConfig, layout = []) {
  ...
  crateLite.typeLabels = crateLite.tabular.typeLabels;
