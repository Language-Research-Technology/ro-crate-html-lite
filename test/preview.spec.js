...
describe('preview.js', function() {
  describe('roCrateToJSON', function() {
    it('should convert a basic RO-Crate to JSON format', async function() {
      ...
      assert.ok(rootEntity.type.includes('Dataset'), 'Root entity should be of type Dataset');
      
      // Test that the name property is properly expanded
      const nameUri = "http://schema.org/name";
      assert.ok(rootEntity.props[nameUri].fwd, 'Name should have forward values');
      assert.equal(rootEntity.props[nameUri].fwd.length, 1, 'Should have one name value');
      assert.equal(rootEntity.props[nameUri].fwd[0].value, 'Test RO-Crate', 'Name should match what we set');
      ...
      assert.equal(fileEntityResult.id, 'test-file.txt', 'File entity should have correct id');
      assert.ok(fileEntityResult.type.includes('File'), 'File entity should be of type File');
      
      // Test that the person entity is included
      const personEntityResult = result.ids['https://orcid.org/0000-0000-0000-0000'];
      assert.ok(personEntityResult, 'Person entity should exist');
      assert.ok(personEntityResult.type.includes('Person'), 'Person entity should be of type Person');
      
      // Test that the author relationship is properly expanded
      const authorUri = Object.keys(rootEntity.props).find(key => 
        rootEntity.props[key].label === 'author' || key === 'author'
      );
      assert.ok(authorUri, 'Should have an author property');
      assert.ok(rootEntity.props[authorUri].fwd, 'Author should have forward values');
      assert.equal(rootEntity.props[authorUri].fwd.length, 1, 'Should have one author value');
      assert.equal(rootEntity.props[authorUri].fwd[0].target_id, 'https://orcid.org/0000-0000-0000-0000', 'Author should link to person entity');
      assert.equal(rootEntity.props[authorUri].fwd[0].target_name, 'Test Author', 'Author should have correct name');
      ...
      assert.ok(result.types.Dataset.includes('./'), 'Dataset type should include root entity');
      assert.ok(result.types.File.includes('test-file.txt'), 'File type should include test file');
      assert.ok(result.types.Person.includes('https://orcid.org/0000-0000-0000-0000'), 'Person type should include test person');
    ...
    it('should handle external URLs in properties', async function() {
      ...
      const licenseUri = Object.keys(fileEntity.props).find(key => 
        fileEntity.props[key].label === 'license' || key === 'license'
      );
      assert.ok(licenseUri, 'Should have license property');
      assert.ok(fileEntity.props[licenseUri].fwd, 'License should have forward values');
      assert.equal(fileEntity.props[licenseUri].fwd[0].url, 'https://creativecommons.org/licenses/by/4.0/', 'License should be external URL');
    ...
    it('Should handle multi-page configuration', async function() {
      ...
      const crateData = JSON.parse(fs.readFileSync("test_data/f2fnew/ro-crate-metadata.json", "utf8"));
      const crate = new ROCrate(crateData, { array: true, link: true });
      const multiPageConfig = {
        types: {
          "RepositoryObject": "repositoryobject-template.html",
          "Person": "person-template.html" 
        }
      };
      ...
      assert.ok(Object.keys(result.pages).length > 0, 'Should have generated pages');
      for (const [entityId, pagePath] of Object.entries(result.pages)) {
        const entity = crate.getEntity(entityId);
        assert.ok(entity, `Entity ${entityId} should exist in crate`);
        const entityTypes = entity['@type'];
        const matchedType = Object.keys(multiPageConfig.types).find(type => entityTypes.includes(type));
        assert.ok(matchedType, `Entity ${entityId} should match a type in multi-page config`);
        const expectedPagePath = "/ro-crate-preview_html/" + quadTreeId(entityId);
        assert.equal(pagePath, expectedPagePath, `Page path for ${entityId} should match expected structure`);
      }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  async function runTests() {
    try {
      ...
      const rootEntity = result.ids[result.entryPoint];
      const nameProperty = Object.values(rootEntity.props).find(prop => 
        prop.label === 'name' && prop.fwd && prop.fwd[0] && prop.fwd[0].value === 'Test RO-Crate'
      );
