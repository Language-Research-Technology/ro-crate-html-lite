  ...
  if (noConfigProvided) {
    ...
    for (const navKey of Object.keys(config.navigationByType)) {
      if (crateLite.types[navKey]) {
        ...
        // First try exact URI match, then a local-name fallback (e.g. http/https variants).
        let bareName = Object.keys(crateLite.typeUrls).find(
          (t) => crateLite.typeUrls[t] === navKey
        );

        if (!bareName) {
          const navKeyLocal = getUriLocalName(navKey).toLowerCase();
          if (navKeyLocal) {
            bareName = Object.keys(crateLite.typeUrls).find((t) =>
              getUriLocalName(crateLite.typeUrls[t]).toLowerCase() === navKeyLocal
            );
          }
        }
...
function shouldSkipReciprocalProperty(entityLite, uri) {
  ...
  return !!(reciprocalUri && entityLite.props[reciprocalUri]);
}

function initializeProp(crate, entityLite, prop, resolvedUri = null) {
...
async function roCrateToJSON(crate, multiPageConfig, layout = []) {
  ...
  for (let entity of crate.entities()) {
    ...
    for (let prop of Object.keys(entity)) {
      ...
      const uri = initializeProp(crate, entityLite, prop, resolvedUri);
 ...
 *
 */
function renderTemplate(data, template, layout, css = "") {
  ...
  return env.renderString(template, { data, layout, css });
}
